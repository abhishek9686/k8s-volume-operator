...
import (
	...
	demov1 "github.com/operator/api/v1"
...
//+kubebuilder:rbac:groups=demo.example.org,resources=demovolumes,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=demo.example.org,resources=demovolumes/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=demo.example.org,resources=demovolumes/finalizers,verbs=update
...
func (r *DemoVolumeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if volume.Spec.Name != volume.Status.Name {
		...
		r.Status().Update(context.Background(), &volume)
	}
	r.reconcilePVC(ctx, &volume, l)
	return ctrl.Result{}, nil
}

func (r *DemoVolumeReconciler) reconcilePVC(ctx context.Context,
	volume *demov1.DemoVolume, l logr.Logger) error {
	...
	storageReq, _ := resource.ParseQuantity(fmt.Sprintf("%dGi", volume.Spec.Size))
	pvc = &apiv1.PersistentVolumeClaim{
